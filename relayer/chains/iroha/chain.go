...
func NewChain(config ChainConfig) (*Chain, error) {
	rpcClient, err := NewRPCClient(config.RpcAddr)
	if err != nil {
		return nil, err
	}

	// create EthereumAddress from IrohaAccountId
	// https://iroha.readthedocs.io/en/main/integrations/burrow_example.html#case-1-running-computations-and-storing-data
	hash := gethcrypto.Keccak256([]byte(config.AccountId))
	relayerAddress := common.HexToAddress(hex.EncodeToString(hash[12:32]))

	ibcHost, err := NewIbcHost(config.IBCHostAddress(), rpcClient)
	if err != nil {
		return nil, err
	}
	ibcHandler, err := NewIbcHandler(config.IBCHandlerAddress(), rpcClient)
	if err != nil {
		return nil, err
	}
	return &Chain{
